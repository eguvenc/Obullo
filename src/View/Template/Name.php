...
namespace Obullo\View\Template;
...
class Name
{
    ...
    public function setName($name)
    {
        ...
        if (count($parts) === 1) {
            ...
            throw new LogicException(
                sprintf(
                    'The template name "%s" is not valid.',
                    $this->name
                )
            );
        }
    ...
    {
        if ($file === '') {
            throw new LogicException('The template name cannot be empty.');
        }
        $this->file = $file;

        if (false == is_null($this->engine->getFileExtension())) {
    ...
    public function getPath()
    {
        ...
        if (false == is_file($path) and $this->folder->getFallback() and is_file($this->getDefaultDirectory() . DIRECTORY_SEPARATOR . $this->file)) {
